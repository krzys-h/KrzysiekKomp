...
void disc_read(u8int disc, u8int head, u16int cylinder, u8int sector, u8int sector_count, u16int* buffer)
{
	...
	u8int _status = 0;
	vga_puts("Oczekiwanie na odpowiedz...\n");
	while(!(_status & 0x08)) _status = inportb(0x1F7);
	vga_puts("Odczytywanie danych...\n");
	u32int _counter = 0;
	u16int* _buffer = buffer;
