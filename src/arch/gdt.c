...
void gdt_init()
{
	gdt_set_desc(&gdt[0], 0, 0, 0, 0);                // NULL - beginning of GDT
	gdt_set_desc(&gdt[1], 0, 0xFFFFFFFF, 0x9A, 0xCF); // OS (ring0) code
	gdt_set_desc(&gdt[2], 0, 0xFFFFFFFF, 0x92, 0xCF); // OS (ring0) data
	gdt_set_desc(&gdt[3], 0, 0xFFFFFFFF, 0xFA, 0xCF); // Applications (ring3) code
	gdt_set_desc(&gdt[4], 0, 0xFFFFFFFF, 0xF2, 0xCF); // Applications (ring3) data
